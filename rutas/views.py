...
def upload_page(request): 
    ...
    if request.method == 'POST':
        ...
        current_path = os.path.join(settings.MEDIA_ROOT, 'tracks')
...
def ruta_page(request, ruta_id):
    ...
    try:  
        ...
        ruta_json = os.path.join(settings.MEDIA_ROOT, 'json', 'aemet.json')      
...
def eliminarRuta_page(request, ruta_id):
    ...
    ruta_track = os.path.join(settings.MEDIA_ROOT, 'tracks', nombre_archivo)
    #print(ruta_track)
    ...
    for imagen in imagenes:
        ...
        ruta_imagen = os.path.join(settings.MEDIA_ROOT, path_image)
        #print(ruta_imagen)
    ...
    for video in videos:
        ...
        ruta_video = os.path.join(settings.MEDIA_ROOT, path_video)
        ruta_miniatura = os.path.join(settings.MEDIA_ROOT, path_miniatura)
...
def subirTrack(track):
    ...
    if track:
        ...
        url_archivo = os.path.join(settings.MEDIA_ROOT, 'tracks', archivo_nombre) 
        ...
        ruta_path = os.path.join(settings.MEDIA_ROOT, 'tracks', ruta_name)
...
def subirImagenesOVideos(ruta, imagenes, videos):
    ...
    if imagenes:
        ...
        for archivo in imagenes:
            ...
            url_imagen = os.path.join(settings.MEDIA_ROOT, 'images', archivo_nombre) 
    ...
    if videos:     
        ...
        for archivo in videos:  
            ...
            archivo_nombre = archivo.name
            output_thumbnail = os.path.join(settings.MEDIA_ROOT, 'videos')
            url_video = os.path.join(settings.MEDIA_ROOT, 'videos',  archivo_nombre) 
...
def obtener_palabras_clave(latitud, longitud):
    ...
    valores_num = ['1', '2', '3', '4', '5', '6', '7', '8', '9', '0']
    ...
    if data['status'] == 'OK':
        ...
        for result in data['results']:
            ...
            for address_component in result['address_components']:
                ...
                if address_component['long_name'] not in valores_unicos_temp:
                    
                    valores_unicos_temp.append(address_component['long_name'])
                    
        for valor in valores_unicos_temp:
            
            if not any(num in valor for num in valores_num):
